...
class LiveConfig(ConfigFactory):
    def create_config(self) -> AppConfig:
        secret_dict = ast.literal_eval(_load_secret())
...
def _load_secret() -> Secret:
    ...
    resp = client.get_secret_value(os.environ.get("SECRET_ID"))
    return Secret(
        secret_name=os.environ.get("SECRET_ID"), secret_string=resp["SecretString"]
    )
